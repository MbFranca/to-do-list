...
import { FaRegStar, FaStar, FaTrash, FaArrowDown, FaArrowUp, FaRegCircleCheck } from 'react-icons/fa6';
...
export default function TaskComplete({ taskComplete, handleDeletTask, handlePending }: TaskListProps) {
...
const clickToComplete = (idTask: string) => {
  ...
  return (
    <div className={`taskCompletedContainer ${taskComplete.length>0 ? '':'hidden'}`}>
          ...
          {taskComplete ? (
            taskComplete.map((task, index) => {
              return (
                ...
                <li className='listItem' key={task.id} ref={(index === taskComplete.length -1) ? listRef : null }>
                  <button
                  ...
                  <input className='taskTittleComplete' defaultValue={task.value} readOnly />
